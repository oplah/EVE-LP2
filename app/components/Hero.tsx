...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 lg:px-8 pt-36 flex flex-col items-center text-center" style={{ paddingBottom: "33px" }}>
        <h1 className="title-bold-1 text-[#1A1A1A] mb-6">
          Meet EVE AI
        </h1>
        <p className="title-6 text-content-secondary max-w-2xl mb-8">
        ...
        <div className="flex items-center gap-4 mb-12">
          ...
          <div className="h-px w-12 flex-shrink-0" style={{ background: "linear-gradient(90deg, #8A87EF, transparent)" }} />
        </div>
      </div>
      ...
      <div className="relative z-10 max-w-5xl mx-auto w-full px-6 lg:px-8 pb-0">
        <div
          className="relative w-full rounded-t-3xl overflow-hidden border border-black/8 border-b-0 bg-[#0D0920] cursor-pointer"
        ...
        >
          ...
          {/* Bottom controls — stop propagation so they don't toggle play */}
          <div className="absolute bottom-4 right-4 flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
            {/* Mute / unmute toggle */}
            <button
              onClick={() => setIsMuted((m) => !m)}
              className="w-9 h-9 rounded-full flex items-center justify-center bg-black/40 hover:bg-black/60 backdrop-blur-sm transition-all duration-200 text-white"
